...
color planeColor;
double refractF;
Plane::Plane(const dvec3 & point, const dvec3 & normal, const color & material, const double refractFactor)
	: ImplicitSurface(material), a(point), n(normalize(normal))
{
	planeColor = material;
	refractF = refractFactor;
}
...
HitRecord Plane::findIntersect(const Ray& ray){
	...
	if (abs(glm::dot(ray.direct, n)) <0.001) {
		...
		if (t < 0) {
			...
			hitRecord.material.emissiveColor = planeColor;
			hitRecord.material.dielecticRefractionIndex = refractF;
