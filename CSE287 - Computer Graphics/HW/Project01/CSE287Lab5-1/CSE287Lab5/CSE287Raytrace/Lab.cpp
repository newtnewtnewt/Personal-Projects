...
void buildScene()
{
	...
	shared_ptr<Sphere> redBall = make_shared<Sphere>(dvec3(4.0, 2.0, -13.0), 2.0, color(1.0, 0.0, 0.0, 0.9), 1.5);
	shared_ptr<Sphere> greenBall = make_shared<Sphere>(dvec3(0.0, 1.0, -15.0), 1.5, color(0.0, 1.0, 0.0, 0.9), 2.0);
	shared_ptr<Sphere> purpleBall = make_shared<Sphere>(dvec3(0.0, 0.0, -10.0), 1.5, color(0.2, 0.2, 0.2, 0.1), 0.7);
	shared_ptr<Sphere> blueBall = make_shared<Sphere>(dvec3(-4.0, 2.0, -13.0), 2.0, color(0.0, 0.0, 1.0, 0.9), 2.5);
	shared_ptr<Plane> whitePlane = make_shared<Plane>(dvec3(0.0, -2.0, 0.0), dvec3(0.0, 1.0, 0.0), color(1, 1, 0.5, 0.9), 0.3);
	shared_ptr<Ellipsoid> yellowEllipsoid = make_shared<Ellipsoid>(dvec3(4.0, 0.0, -11.0), color(0.5, 0.5, 0.0, 0.9), 0.75, 1.0, 0.3);
	shared_ptr<Cylinder> turqCylinder = make_shared<Cylinder>(dvec3(-4.0, 0.0, -8.0), color(64.0 / 255.0, 224 / 255.0, 208 / 255.0, 0.9), 1.0, 1.0, 
		X_AXIS);
	rayTrace.surfaces.push_back(redBall);
	rayTrace.surfaces.push_back(greenBall);
	rayTrace.surfaces.push_back(blueBall);
	rayTrace.surfaces.push_back(purpleBall);
	rayTrace.surfaces.push_back(whitePlane);
	rayTrace.surfaces.push_back(yellowEllipsoid);
	rayTrace.surfaces.push_back(turqCylinder);

	// Create light sources and add them to the scene.
	ambientLight = make_shared<LightSource>(color(0.15, 0.15, 0.15, 1));
	...
	lightSpot = make_shared<SpotLight>(dvec3(0, 3, -13), dvec3(0, -1, 0), PI / 4 , color(0.6, 0.6, 0.6, 1));
