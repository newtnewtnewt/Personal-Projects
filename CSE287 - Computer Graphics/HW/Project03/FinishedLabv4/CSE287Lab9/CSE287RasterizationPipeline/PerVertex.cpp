...
void PerVertex::applyLocalIllumination(VertexData& vertData)
{
	if (PerVertex::perVertexLightingEnabled) {	
		...
		double alpha = vertData.shadedColor.a;
		for (auto* light : lights) {
			totalColor += light->getLocalIllumination(glm::normalize(eyePositionInWorldCoords - vertData.worldPosition), vertData.objectPos, vertData.objectNormal, vertData.material, vertData.uv);
		}
		vertData.shadedColor = totalColor;
		vertData.shadedColor.a = alpha;
	}
