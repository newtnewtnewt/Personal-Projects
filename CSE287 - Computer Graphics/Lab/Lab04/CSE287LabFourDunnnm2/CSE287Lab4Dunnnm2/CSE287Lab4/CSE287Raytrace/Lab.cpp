...
// Raytracer
RayTracer rayTrace(frameBuffer, LIGHT_BLUE );  // Default color if no object is hit
...
void buildScene()
{
	...
	shared_ptr<Sphere> greenBall = make_shared<Sphere>(dvec3(5.0, 6.0, -10.0), 3.0, GREEN);
	std::vector<dvec3> dim = { dvec3(0.0, 0.0, -11.0), dvec3(1.0, 0.0, -11.0), dvec3(1.0, 1.0, -11.0) };
	shared_ptr<Plane> bluePlane = make_shared<Plane>(dvec3(0.0, -2.0, 0.0), dvec3(0.0, 1.0, 0.0), BLUE);
	
	rayTrace.surfaces.push_back(redBall);
	rayTrace.surfaces.push_back(greenBall);
	rayTrace.surfaces.push_back(bluePlane);
