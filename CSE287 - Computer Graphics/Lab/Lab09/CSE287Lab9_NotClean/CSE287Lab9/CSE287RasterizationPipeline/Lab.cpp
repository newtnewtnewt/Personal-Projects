...
static glm::dmat4 myPerspective(double fovy, double aspect, double nearby, double faraway) {
	...
	perspectiveM[2][2] = faraway / (faraway - nearby);
	perspectiveM[2][3] = -1;
	perspectiveM[3][2] = -(faraway * nearby) / (faraway - nearby);
	return perspectiveM;
...
static dmat4 myOrtho(double left, double right, double bottom, double top, double zClose, double zFar) {
	dmat4 orthoM;
	orthoM[0][0] =  (2.0 / (left - right));
	orthoM[1][1] =  (2.0) / (top - bottom);
	orthoM[2][2] =  (-2) / (zFar - zClose);
	orthoM[2][3] = -2/(zFar - zClose);
...
void twoViewsSplitVertically()
{
	...
	PerVertex::projectionTransformation = myOrtho(-7.0, 7.0, -6.0, 6.0, 0, 100.0);
...
void twoViewsSplitHorizontally()
{
	...
	PerVertex::projectionTransformation = myOrtho(-14.0, 14.0, -8.0, 8.0, 15, 40.0);
