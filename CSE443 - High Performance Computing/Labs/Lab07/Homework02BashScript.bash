...
g++ -o workhorse_explicit workhorse_explicit.cpp -O3 --std=c++14 -Wall -fopenmp
...
for i in {1..2}
	do
		/usr/bin/time -f "%U %S %e" ./workhorse  2>> allTestRuns.txt
	done
...
for x in {1..8}
	do 
		...
		do
			...
			/usr/bin/time -f "%U %S %e" ./workhorse_explicit `x` 2>> allTestRuns.txt
		done
    done
