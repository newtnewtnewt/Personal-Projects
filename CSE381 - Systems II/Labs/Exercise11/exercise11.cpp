...
std::atomic<int> currID = ATOMIC_VAR_INIT(0);
...
void producer(std::vector<int>& q, std::mutex& qMutex,
    ...
    for (int i = 0; i < n; i++) {
        // Implement this method so that it works consistently with the
        // consumer method (see further below)
        std::unique_lock<std::mutex> lock(qMutex);
        // Wait until queue is not full
        qCondVar.wait(lock, [&q, &n]{ return q.size() < 5; });
...
std::vector<int> countDigits(const std::vector<int> values, int n) {
    ...
    auto thrMain = [&](int id) {
            for (size_t i = id; (i < values.size()); i += n) { 
                std::lock_guard<std::mutex> guard(lockControl);
                counts[values[i] % 10]++;
            }
        };
